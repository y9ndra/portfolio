...
export const PROJECTS = [
  {
    ...
    problem: [
      ...
      },
      {
        title: "Approach",
        description:
          "I architected Sotrix as a <strong>backend-first, decoupled system</strong> built on <strong>Node.js, Express, and MongoDB</strong>, establishing clean boundaries between synchronous client traffic and asynchronous background computation:<br/>• <strong>Real-Time Communication:</strong> Integrated <strong>Socket.IO</strong> with a <strong>Redis</strong> pub/sub adapter to power instant messaging, live presence tracking, and typing indicators without bottlenecking web processes.<br/>• <strong>Asynchronous Worker Pipeline:</strong> Used <strong>BullMQ</strong> on <strong>Redis Cloud</strong> to isolate CPU-intensive media transformations (compression & formatting via Sharp) and notification fan-outs from the core request-response thread.<br/>• <strong>Hardened Security & Auth:</strong> Implemented a stateless <strong>dual-token authentication lifecycle</strong> (short-lived access tokens paired with rotating refresh tokens in <code>HttpOnly</code> cookies) and enforced strict <strong>Zod</strong> runtime validation across 45+ endpoints.<br/>• <strong>Efficient Data Access:</strong> Designed optimized MongoDB schema indexing, cursor-based pagination, and Redis caching layers to ensure low-latency data access even under concurrent load."
      }
    ],
    architecture: [
    ...
    learned:
      "Building Sotrix shifted my perspective from writing standalone endpoints to <strong>reasoning about backend components as a cohesive, resilient system</strong>. I gained deep practical experience navigating the architectural trade-offs essential for scale: isolating heavy media computation into distributed job queues to prevent event-loop starvation, synchronizing real-time WebSocket state with client-side query caches to eliminate race conditions, and defending authentication lifecycles against modern web attack vectors.<br/><br/>From tuning Redis cache policies and database pagination strategies to designing clean, contract-tested RESTful APIs, Sotrix solidified my ability to design and build production-ready backend infrastructure from the ground up."
  },
  {
    id: "spendwise",
    title: "SpendWise",
