    ...
    problem: [
      {
        title: "Problem",
        description:
          "Most tutorials and small projects only teach isolated features in a vacuum, but that's not how real software works. I wanted to build a complete <strong>end-to-end application from scratch</strong> and understand everything that connects across the stack. My goal was to stand in the shoes of a developer working on a <strong>production-level application</strong>, tackling challenges like real-time communication, background worker queues, secure authentication, and database performance within a single cohesive product."
      },
      {
        title: "Approach",
        description:
          "I architected Sotrix as a <strong>backend-first, decoupled system</strong> built on <strong>Node.js, Express, and MongoDB</strong>, establishing clean boundaries between synchronous client traffic and asynchronous background computation:<br/>• <strong>Real-Time Communication:</strong> Implemented bi-directional messaging with <strong>Socket.IO</strong> and a <strong>Redis Pub/Sub</strong> bridge, powering instant chat, live user presence, and typing indicators while decoupling background worker events from the main web process.<br/>• <strong>Asynchronous Worker Pipeline:</strong> Used <strong>BullMQ</strong> on <strong>Redis Cloud</strong> to isolate CPU-intensive media transformations (compression & formatting via Sharp) and notification fan-outs from the core request-response thread.<br/>• <strong>Hardened Security & Auth:</strong> Implemented a stateless <strong>dual-token authentication lifecycle</strong> (short-lived access tokens paired with rotating refresh tokens in <code>HttpOnly</code> cookies) and enforced strict <strong>Zod</strong> runtime validation across 45+ endpoints.<br/>• <strong>Efficient Data Access:</strong> Designed optimized MongoDB schema indexing, cursor-based pagination, and Redis caching layers to ensure low-latency data access even under concurrent load."
      }
    ],
    architecture: [
      "<strong>Frontend</strong> - React 19 · TypeScript · Vite · Zustand · TanStack Query · Motion micro-interactions",
      "<strong>Real-Time</strong> - Socket.IO gateway · Redis Pub/Sub bridge · presence heartbeats · debounced typing channels",
